...
import { CallsHistoryModel } from "../models/CallsHistory";
    ...
    if (event.event == "Hangup") {
      ...
      try {
        ...
        if (callToClose?.status == "Abandonado") {
          console.warn(
            `A chamda de id ${callToClose.linkId} já foi abandonada na fila ${callToClose.queue}.`,
          );
          return;
        }
        ...
        if (callToClose?.status == "Chamando") {
          console.warn(
            `A chamada de id ${callToClose.linkId} ainda está aguardando na fila ${callToClose.queue}.`,
          );
          return;
        }
    ...
    if (event.event == "QueueCallerJoin") {
      ...
      try {
        await updateStatus({
          ...
          status: "Atendido",
          show: true,
          extension: event?.destchannel?.slice(6, 13) || "",
        });
        const response = await getThisCall(event.linkedid);
        if (response) {
          return response;
